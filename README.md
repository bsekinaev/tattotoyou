...
сеанса, длительность, согласованную цену, сумму и статус предоплаты. PostgreSQL
exclusion constraint запрещает пересечение активных сеансов со статусами
`pending` и `confirmed`, поэтому конфликт невозможно обойти параллельными
запросами к UI.
